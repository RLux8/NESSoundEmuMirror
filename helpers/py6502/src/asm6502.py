...
class asm6502(object):
        ...
        for i in range(len(self.allstuff)):
            ...
            if (lowbyte == -1) and (addressmode == "relative"):
                ...
                if (value in self.symbols):
                    ...
                    lowbyte = newvalue & 0x0ff
                # some ugly hacks here....
                elif (value.split("+")[0] in self.symbols):
                     lowbyte = (self.symbols[value.split("+")[0]] + self.decode_value(value.split("+")[1])) & 0xFF

                if (highbyte == -1) and (addressmode in self.modeswithhighbytevalue):
                    if (value.split("+")[0] in self.symbols):
                        newvalue = self.symbols[value.split("+")[0]]
                        
			# some ugly hacks here....
                        if len(value.split("+")) != 1 and ((self.symbols[value.split("+")[0]] & 0xFF) + self.decode_value(value.split("+")[1])) > 0xFF:
                            newvalue += 0xFF
    ...
    def srecord_checksum(self, astring):
        checksum = 0
        for i in range(old_div(len(astring), 2)):
